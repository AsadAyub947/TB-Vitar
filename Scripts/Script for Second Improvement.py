"""
TB-ViTAR
Second Improvement F: Process-Reward GRPO + Full TARA loop
"""
...
def setup_dataset():
    ...
    for ext in ("jpg","jpeg","png"):
        for p in tbx_root.rglob(f"*.{ext}"): 
            IMAGE_INDEX[p.stem]=str(p); IMAGE_INDEX[p.name]=str(p)
    
    rows = []
    for ext in ("jpg","jpeg","png"):
        for p in tbx_root.rglob(f"*.{ext}"):
            # Get immediate parent folder name only
...
# ── Second Improvement F: Process-Reward GRPO with Full TARA loop ──────────────────────────────────
@app.function(
    gpu="A100-80GB",
    timeout=5*3600,
    volumes={DATASET_MOUNT_PATH: vol},
)
def run_second_improvement_F():
    exec(SHARED, globals())
    ...
    print("Starting Process-Reward GRPO with Full TARA loop (Second Improvement F) ...")
    ...
    print(f"Second Improvement F saved -> {PROCESS_CKPT_DIR}")
    if reward_log:
        last50=reward_log[-50:]
        print(f"Second reward stats: mean={sum(last50)/len(last50):.3f} "
    ...
    print("\n[Second Improvement F — Process GRPO] RESULTS:")
...
def main():
    print("="*60)
    print("Second Improvement F: Process-Reward GRPO (TARA loop)")
    print("="*60)
    res_f = run_second_improvement_F.remote()
    print("\nSecond Improvement F DONE:")
