"""
TB-ViTAR
First Improvement F: PEFT LoRA + Process-Reward GRPO (full TARA loop)
"""
...
# ── First Improvement F: Process-Reward GRPO ──────────────────────────────────
@app.function(
    gpu="A100-80GB",
    timeout=5*3600,
    volumes={DATASET_MOUNT_PATH: vol},
)
def run_first_improvement_F():
    exec(SHARED, globals())
    ...
    print("Starting Process-Reward GRPO (First Improvement F) ...")
    ...
    print(f"First Improvement F saved -> {PROCESS_CKPT_DIR}")
    if reward_log:
        last50=reward_log[-50:]
        print(f"First reward stats: mean={sum(last50)/len(last50):.3f} "
    ...
    print("\n[First Improvement F — Process GRPO] RESULTS:")
...
def main():
    print("="*60)
    print("First Improvement F: Process-Reward GRPO (TARA loop)")
    print("="*60)
    res_f = run_first_improvement_F.remote()
    print("\nFirst Improvement F DONE:")
